...
if($_POST["playlist"])
{
	...
	*/
	...
	THIS FIXES ISSUE 3: Playlist Name is wierd (https://github.com/navinpai/8Tracks-Downloader/issues/3)
		
	KNOWN ISSUE: For newly created playlists, default title (Untitled Mix) and description (- handcrafted internet radio from 8tracks.) are shown
	
	*/
		$tags = get_meta_tags($_POST["playlist"]);
		$title=explode(',',$tags['keywords']);
		$desc = array_map('strrev', explode('-', strrev($tags['description'])));
		echo '<div class="title"><h3>'.$title[0].'</h3></div><br/><div class="desc"><h4>'.$desc[1];
		echo '</h4></div><br/><br/><br/>';
	...
	*/
	...
	$obj = json_decode($songdata,true);
	$oldsong='songs/'.$obj['set']['track']['name'].'.m4a';
	unlink($oldsong);
